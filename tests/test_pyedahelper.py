...
def test_method_str():
    """
    Tests if the function correctly throws an error if the method is not a string
...
def test_cols_int():
    ...
    """
    with pytest.raises(AssertionError, match = "Cols must be a list or a string!"):
...
def test_non_numeric():
    """
    Tests if the function correctly throws an error if an column in cols is non-numeric
...
def test_remove_method():
    ...
    assert sample_drop.shape[0] < test_data.shape[0] #should have fewer rows because NAs are removed
    assert sample_drop.shape[1] == test_data.shape[1] #number of columns should not change
...
def test_median():
    ...
    assert sample_median.shape == test_data.shape #dimensions should not change
...
def test_mode():
    ...
    assert sample_mode.shape == test_data.shape #dimensions should not change