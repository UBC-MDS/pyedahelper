...
def test_inputs():
    ...
    with pytest.raises(AssertionError,
                       match="Data must be in pandas Data Frame!"):
        pyedahelper.fast_plot(df=[1, 2], x="col_date", y="col_int",
                              plot_type="line")

    # x input isn't string
    with pytest.raises(AssertionError,
                       match="x column name must be a string!"):
        pyedahelper.fast_plot(df=df, x=["col_date"], y="col_int",
                              plot_type="line")

    # y input isn't string
    with pytest.raises(AssertionError,
                       match="y column name must be a string!"):
        pyedahelper.fast_plot(df=df, x="col_date", y=["col_int"],
                              plot_type="line")

    # x input is not a column in data frame
    with pytest.raises(AssertionError,
                       match="x column name is "
                             "not a column in data frame entered!"):
        pyedahelper.fast_plot(df=df, x="asdfjsf", y="col_int",
                              plot_type="line")

    # y input is not a column in data frame
    with pytest.raises(AssertionError,
                       match="y column name is "
                             "not a column in data frame entered!"):
        pyedahelper.fast_plot(df=df, x="col_date", y="slkfjas",
                              plot_type="line")

    # plot type not bar/line/scatter
    with pytest.raises(AssertionError,
                       match='plot_type must be either:'
                             ' "scatter", "line",  or "bar"'):
        pyedahelper.fast_plot(df=df, x="col_date", y="col_int",
                              plot_type="slfjalskf")
    # x column should not be all null
    with pytest.raises(AssertionError, match="x Column must not be all Null!"):
        pyedahelper.fast_plot(df=df, x="col_nan", y="col_int",
                              plot_type="line")

    # x column should not be all null
    with pytest.raises(AssertionError, match="y Column must not be all Null!"):
        pyedahelper.fast_plot(df=df, x="col_int", y="col_nan",
                              plot_type="line")
...
def test_scatter():
    ...
    """
    a = pyedahelper.fast_plot(df=df, x="col_int", y="col_flt",
                              plot_type="scatter")
...
def test_bar():
    ...
    with pytest.raises(Exception):
        pyedahelper.fast_plot(df=df, x="col_date", y="col_date",
                              plot_type="bar")
    with pytest.raises(Exception):
        pyedahelper.fast_plot(df=df, x="col_chr", y="col_chr", plot_type="bar")
    with pytest.raises(Exception):
        pyedahelper.fast_plot(df=df, x="col_date", y="col_chr",
                              plot_type="bar")
    with pytest.raises(Exception):
        pyedahelper.fast_plot(df=df, x="col_chr", y="col_date",
                              plot_type="bar")
